...
from .mygpt_utils import md5sum
from .prompts_chain import *
from .parser import read_doc
...
from .response_data import Answer, Context
# from .utils import maybe_is_text, md5sum


# TODO: introduce caching logic
...
class Docs:
    ...
    def add(
        ...
        print(f'{len(self.docs)} documents ingested in total')
    ...
    async def aquery(
        ...
        for c in contexts:
            ...
            text = c.context
            citation = c.citation
            # do check for whole key (so we don't catch Keya with Key)
            skey = key.split(" ")[0]
